...
lapply(file_list[start_id:length(file_list)], function(file_name) {
    ...
    text[hist_id %like% "----",c("hist_id_1","hist_id_2","hist_id_3"):=unique(tstrsplit(hist_id,"----")), 
    ...
    dt_final <- text[, .(date_processed, prod_desc, hist_id_1, hist_price_1, hist_id_2, hist_price_2, hist_id_3,
    ...
    setnames(dt_final, c("date_processed", "prod_desc", "historical product ID #1", "historical price #1", 
        "historical product ID #2", "historical price #2","historical product ID #3","source order file name",
    ...
    #remove id from text 
    #-----------------------------------------#
    dt_final[!is.na(get("historical product ID #1")), id_temp:=gsub(" ", "",get("historical product ID #1")),
        by=1:nrow(dt_final[!is.na(get("historical product ID #1"))])]
    dt_final[!is.na(get("historical product ID #1")), id_temp:=paste0(strsplit(get("id_temp"),"")[[1]], 
        collapse=paste0("(",product_id_sep, ")*")),by=1:nrow(dt_final[!is.na(get("historical product ID #1"))])]
    dt_final[!is.na(get("historical product ID #1")), c("prod_desc"):=gsub(id_temp, "", get("prod_desc")),
        by=1:nrow(dt_final[!is.na(get("historical product ID #1"))])]

    dt_final[!is.na(get("historical product ID #2")), id_temp:=gsub(" ", "",get("historical product ID #2")),
        by=1:nrow(dt_final[!is.na(get("historical product ID #2"))])]
    dt_final[!is.na(get("historical product ID #2")), id_temp:=paste0(strsplit(get("id_temp"),"")[[1]], 
        collapse=paste0("(",product_id_sep, ")*")),by=1:nrow(dt_final[!is.na(get("historical product ID #2"))])]
    dt_final[!is.na(get("historical product ID #2")), c("prod_desc"):=gsub(id_temp, "", get("prod_desc")),
        by=1:nrow(dt_final[!is.na(get("historical product ID #2"))])]

    dt_final[!is.na(get("historical product ID #3")), id_temp:=gsub(" ", "",get("historical product ID #3")),
        by=1:nrow(dt_final[!is.na(get("historical product ID #3"))])]
    dt_final[!is.na(get("historical product ID #3")), id_temp:=paste0(strsplit(get("id_temp"),"")[[1]], 
        collapse=paste0("(",product_id_sep, ")*")),by=1:nrow(dt_final[!is.na(get("historical product ID #3"))])]
    dt_final[!is.na(get("historical product ID #3")), c("prod_desc"):=gsub(id_temp, "", get("prod_desc")),
        by=1:nrow(dt_final[!is.na(get("historical product ID #3"))])]
    ...
    if (file.exists (gsub("\\.csv", paste0("_identified_", output_id, ".csv"), 
        ...
        #     check.names=F)
        # dt_final_orig            <- fread(gsub("\\.csv", 
        #     "_all.csv", output_file), stringsAsFactors=F)
